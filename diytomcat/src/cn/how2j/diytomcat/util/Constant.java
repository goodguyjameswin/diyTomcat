...
public class Constant {
    ...
    public static final File rootFolder = new File(webappsFolder, "ROOT"); // 存放默认web应用
    public static final File confFolder =new File(SystemUtil.get("user.dir"), "conf"); // 各配置文件目录
    public static final File serverXmlFile = new File(confFolder, "server.xml"); // tomcat各内置对象
    public static final File webXmlFile = new File(confFolder, "web.xml"); // tomcat的默认配置
    public static final File contextXmlFile = new File(confFolder, "context.xml"); // web应用默认配置信息
    // 当一个 jsp 被转译成为 .java 文件之后，会被保存在 %TOMCAT_HOME%/ work 这个目录下
