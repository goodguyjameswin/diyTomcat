...
public class HttpProcessor {
    ...
    public void execute(Socket s, Request request, Response response) {
        try {
            ...
            // 通过servlet类名实例化对象，然后调用其doGet方法
            String servletClassName = context.getServletClassName(uri);
            HttpServlet workingServlet; // Servlet的service方法是在chain里面调用，所以用一个workingSerlvet分别指向它们
            ...
            if (request.isForwarded()) { // 防止服务端进行多次跳转，导致程序使用已经关闭的socket从而抛出异常
                return;
            }
            // 根据返回的响应代码，调用不同的处理方法
...
//                return;
    ...
    private void handle200(Socket s, Request request, Response response)
        ...
        if (gzip)
            body = ZipUtil.gzip(body); // 进行gzip压缩
    ...
    protected void handle500(Socket s, Exception e) {
        try {
            OutputStream os = s.getOutputStream();
            StackTraceElement[] stes = e.getStackTrace(); // 拿到Exception的异常堆栈，可以看到依次调用的类的方法的信息
    ...
    private boolean isGzip(Request request, byte[] body, String mimeType) { // 是否进行gzip压缩
